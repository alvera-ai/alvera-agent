...
description: >
  ...
  Real domain orchestration replaces this stub when AR integration coverage
  lands. Use when the user says "set up accounts-receivable", "provision an
  AR tenant", or similar.
---
...
AR-specific suggestions. The primitive skills (`guided`,
`custom-dataset-creation`, `DAC-upload`, `agentic-workflow-creation`,
`query-datasets`) all work for AR resources today — they're
...
2. **Set expectations explicitly.** Tell the user this is a stub —
   ...
   sequence today, point at the healthcare skill's `references/order-walk.md`
   as a reference template they can adapt.
3. **Suggest typical AR resource types.** Common AR shapes:
   - **Custom datasets** — invoice headers, payment events, dunning
     notes. Use `/custom-dataset-creation`.
   - **Tools** — payment-gateway webhooks, email tools, internal
     REST hooks. Use `/guided`.
   - **Workflows** — dunning escalation, payment reconciliation,
     overdue alerts. Use `/agentic-workflow-creation`.
4. **Hand off to `/guided`** for the resource loop. Don't try to
   sequence — that's what the healthcare skill does and that machinery
   isn't ready for AR yet.

## When this stub gets replaced

When `tests/accounts_receivable/_order.json` grows beyond `["smoke"]`,
